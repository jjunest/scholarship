	...
	var touchSlider = {
		...
		init : function () {
			...
			for(var i=0; i<this._len; ++i) {
				if(i==this._len-1 && this.opts.left_side_view > 1){
					this._pos[i] = this._item_w * -1;
				}else{
		...
		position : function (d) {
			...
			if(this.opts.roll) {
				...
				if(this.opts.right_side_view > 1 && this._view <= 1){
					max_chk = this._len - (this.opts.right_side_view+1);
				}else if(this._view <= 1){
					max_chk = this._len - 1;
				}
				if((d == 1 && tmp_pos[this.opts.left_side_view] >= 0) || (this._drag && tmp_pos[this.opts.left_side_view] > 0)) {
		...
		direction : function () {
			...
			if(this.opts.view_more > 0) r = r * this.opts.view_more;
